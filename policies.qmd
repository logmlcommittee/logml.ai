...
## Speakers, Mentors, and Participants
​
...
Each project proposal is evaluated by at least 4 organizers, who assign a score based on the quality of the project submission, and qualifications of the applicant based on mentorship and research experience, as well as the mentor-project fit. To ensure impartiality, the initial project evaluation is conducted blindly, independent of the applicant’s personal or professional background.
...
This document states the Code of Conduct of the LOGML 2026 Workshop (hereafter referred to as “the Event”).
...
Refunds will be granted on a case by case basis and only if the organisation committee is notified before the registration deadline and the relevant supporting evidence is provided.

If a visa application is rejected, we offer an 80% refund of the registration fee. We will process this refund only if the official rejection reason from the immigration authorities shows that the denial was for reasons beyond the participant's control.

We can also provide refunds if the visa is not granted by the registration deadline, and the participant applied within 2 weeks from the acceptance email. Note that once a refund is requested, the participant won't be re-admitted if the visa arrives.

Please note: The 20% we keep covers the cost of holding the participant's spot. We can only process refunds for participants who provide the required proof by the deadline.
